...
Rules for anyone (human or AI agent) working on Kern repositories. These apply to every repo in the KernALO org.

## We build in the open (even while private)
The repos are **private until the v1.0 launch, then they go public with their full history**. Therefore work as if the whole world is watching already:
...
- Keep commit history clean and meaningful — it becomes public.
...
- Push to `origin main` (private phase). Never force-push. If `git pull --rebase` complains about unstaged files that aren't yours (parallel agents share worktrees), use `git -c rebase.autoStash=true pull --rebase`.

## Layout & workflow
- Umbrella dev workspace: `kern/` with sibling repos cloned under `kern/repos/<name>` (gitignored there). pnpm links all `@kernalo/*` packages via the umbrella workspace.
...
- Contracts first: changes to `@kernalo/contracts` / module contracts land (and build) before their consumers.
